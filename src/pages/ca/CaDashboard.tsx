...
const DOC_LABELS: Record<string, string> = {
  ...
  investment_proof: 'Investment Proof', '15ca_pdf': 'Form 15CA',
  '15cb_pdf': 'Form 15CB', other: 'Other',
}
...
function ApproveModal({ id, onClose, onDone }: { id: string; onClose: () => void; onDone: () => void }) {
  ...
  async function submit() {
    if (!cbNumber || remarks.length < 10) {
      setError('15CB number and remarks (min 10 chars) are required.')
      return
    }
    setLoading(true)
    setError('')
  ...
  return (
    <Modal title="Approve — Issue 15CB" onClose={onClose}>
      {error && <ErrBox msg={error} />}
      <Field label="15CB Certificate Number">
        <input className="input-field" style={{ display: 'block' }} value={cbNumber} onChange={e => setCbNumber(e.target.value)} placeholder="e.g. 15CB/2026/001" />
      </Field>
      <Field label="CA Remarks (min 10 chars)">
        <textarea className="input-field" style={{ display: 'block', minHeight: 80, resize: 'vertical' }} value={remarks} onChange={e => setRemarks(e.target.value)} placeholder="Tax compliance confirmed. TDS verified in 26AS…" />
      </Field>
      <ModalActions>
        <button style={S.btn('#27AE60')} onClick={submit} disabled={loading}>{loading ? 'Approving…' : 'Approve & Issue 15CB'}</button>
    ...
    try {
      ...
      const confirmRes = await caFetch(`/ca/compliance/${id}/confirm-pdf`, {
        ...
        body: JSON.stringify({
          ...
          docLabel: label || (docType === '15cb_pdf' ? 'Form 15CB' : 'Form 15CA'),
        }),
      })
    ...
    <Modal title="Upload PDF to Customer Wallet" onClose={onClose}>
      ...
      <Field label="Document Type">
        <select className="input-field" style={{ display: 'block' }} value={docType} onChange={e => setDocType(e.target.value as '15cb_pdf' | '15ca_pdf')}>
          <option value="15cb_pdf">Form 15CB (CA Certificate)</option>
          <option value="15ca_pdf">Form 15CA (Filed Copy)</option>
        </select>
      </Field>
      <Field label="Label (optional)">
        <input className="input-field" style={{ display: 'block' }} value={label} onChange={e => setLabel(e.target.value)} placeholder="e.g. 15CB Certificate — Transfer RH-2026-001" />
      </Field>
      <Field label="PDF File">
        <input type="file" accept=".pdf" onChange={e => setFile(e.target.files?.[0] ?? null)} style={{ fontSize: '0.82rem', color: '#FAF6F0' }} />
      </Field>
      <ModalActions>
...
function File15CAModal({ id, onClose, onDone }: { id: string; onClose: () => void; onDone: () => void }) {
  ...
  async function submit() {
    if (!caNumber.trim()) { setError('15CA number is required.'); return }
    setLoading(true); setError('')
    try {
  ...
  return (
    <Modal title="Record 15CA Number" onClose={onClose}>
      {error && <ErrBox msg={error} />}
      <Field label="15CA Acknowledgement Number">
        <input className="input-field" style={{ display: 'block' }} value={caNumber} onChange={e => setCaNumber(e.target.value)} placeholder="e.g. 15CA/2026/XXXXXXXX" />
      </Field>
      <ModalActions>
        <button style={S.btn()} onClick={submit} disabled={loading}>{loading ? 'Saving…' : 'Save 15CA Number'}</button>
        <button style={S.btn('transparent', '#8BA0B4')} onClick={onClose}>Cancel</button>
      </ModalActions>
    </Modal>
  )
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: '0.72rem', color: '#8BA0B4' }}>
              ...
              {request.fifteen_cb_number && ` · 15CB: ${request.fifteen_cb_number}`}
              {request.fifteen_ca_number && ` · 15CA: ${request.fifteen_ca_number}`}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', flexShrink: 0 }}>
            ...
            {request.status === 'approved' && (
              <>
                ...
                <button style={S.btnOutline} onClick={() => setModal('15ca')}>File 15CA</button>
              </>
            )}
